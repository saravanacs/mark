...
import json
#from simpleerp.SimpleERP.ERP.custom_views.master import item
...
def item_list_autocomplete(request):
    if request.is_ajax():
        ...
        for item in items:
            ...
            item_json['value'] = item.item_code +" "+item.name
    ...
    if request.is_ajax():
        ...
        with connection.cursor() as cursor:
            ...
            for row in rows:
                ...
                item_code=row[2]
                sales_unit=row[3]
                unit_name=Unit.objects.get(pk=sales_unit).unit_name
                company_id=session_user_company(request);

                try:
                    price=Price.objects.get(item=item_id,company=company_id,deleted=0).selling_price_with_tax
                except:
                    price=None
                    pass
                tax_group_id=row[4]
                ...
                if warehouse:
                    try:
                        stock=Stock.objects.get(warehouse=warehouse,item=item_id,deleted=0)
                ...
                return_json['current_stock'] =current_stock
                return_json['unit_name'] =unit_name
                return_json['price'] =price
                if mode==1:
