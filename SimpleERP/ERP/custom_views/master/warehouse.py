...
def list_warehouse(request):
     ...
     try:
        if 'warehouse_name' in request.data.keys() and request.data['warehouse_name']:
            custom_filter['warehouse_name__contains']=request.data['warehouse_name']
        if 'company' in request.data.keys() and  request.data['company']:
