...
def supplier_create(request):
	if request.method == 'GET':
		return Response({'data':'', 'module':'Supplier'}, template_name='ERP/master/supplier/create_update.html')
	else:
		...
		if serializer.is_valid():
			...
			if request.accepted_renderer.format == 'html':
				return Response({"success_data": "Data added successfully"}, template_name='ERP/master/supplier/create_update.html')
			...
			if request.accepted_renderer.format == 'html':
				return Response({"error_data": data}, template_name='ERP/master/supplier/create_update.html')
...
def supplier_view(request, id):
    data_obj = Supplier.objects.get(id=id)
    if request.method == "GET":
        ser_data = SupplierSerializer(data_obj).data
        if request.accepted_renderer.format == 'html':
            return Response({"data": ser_data,"view_mode":1}, template_name='ERP/master/supplier/create_update.html')
...
def supplier_update(request, id):
    ...
    if request.method == "GET":
        ser_data = SupplierSerializer(data_obj).data
        if request.accepted_renderer.format == 'html':
            return Response({"data": ser_data}, template_name='ERP/master/supplier/create_update.html')
        return Response({"data": ser_data}, status=status.HTTP_200_OK)
    else:
        serializer = SupplierSerializer(data_obj, data=request.data, partial=True)
        if serializer.is_valid():
            user_id= session_user_id(request)
            ...
            if request.accepted_renderer.format == 'html':
                return HttpResponseRedirect(reverse('ERP:supplier_list'))
...
def supplier_delete(request,id):
    selected_values=Supplier.objects.get(pk=id)
    user_id= session_user_id(request)
    date_modified=store_date_time()
...
def supplier_list(request):
	...
	try:
	  ...
	  if 'primary_contact_no' in request.data.keys() and request.data['primary_contact_no']:
	      custom_filter['primary_contact_no']=request.data['primary_contact_no']
	  if 'suppliergroup' in request.data.keys() and request.data['suppliergroup']:
	      custom_filter['suppliergroup']=request.data['suppliergroup']
	except Error:
		pass
	model_obj = Supplier.objects.filter(**custom_filter)
	model_data = SupplierSerializer(model_obj, many=True).data
	...
	if request.accepted_renderer.format == 'html':
	   return Response({"data": model_data,'module':'Supplier',"custom_filter":custom_filter}, template_name='ERP/master/supplier/list.html')
