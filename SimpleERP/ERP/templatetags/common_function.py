...
def default_customer_id():
    default_customer=Customer.objects.get(default_cash_customer=1,deleted=0)
    return default_customer.id
             
@register.simple_tag
def default_customer_name():
    default_customer=Customer.objects.get(default_cash_customer=1,deleted=0)
    return default_customer.name
...
@register.simple_tag
def contactpersonlist():
 contact_person_obj = Customer.objects.all()
 contact_person_data = CustomerSerializer(contact_person_obj, many=True).data
 
 return contact_person_data;


@register.simple_tag
def contactpersoncontactnolist():
 contact_person_contact_obj = Customer.objects.all()
 contact_person_contact_data = CustomerSerializer(contact_person_contact_obj, many=True).data
 
 return contact_person_contact_data;
...
@register.simple_tag
def customerlista(id):
 customer_obj=Customer.objects.get(id=id)
 return customer_obj.name;


@register.simple_tag
def contactpersonlista(id):
 person_obj=Customer.objects.get(id=id)
 return person_obj.contact_person;


@register.simple_tag
def contactpersonnolista(id):
 personno_obj=Customer.objects.get(id=id)
 return personno_obj.contact_person_contact_no;